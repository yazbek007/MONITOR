...
TRADING_SETTINGS = {
    ...
    'base_trade_amount': 3,  # 3 USD
    'leverage': 50,  # 50x leverage
    'position_size': 3 * 50,  # 150 USD position size
...
class NtfyNotifier:
    ...
    def __init__(self, topic=None):
        ...
        if not self.topic:
            logger.warning("Ntfy topic not configured. Notifications will be disabled.")
            self.base_url = None
            self.enabled = False
        else:
    ...
    def test_connection(self):
        ...
        try:
            ...
            # Try a simple GET request to ntfy.sh
            test_url = "https://ntfy.sh"
            response = requests.get(test_url, timeout=10)
            
            if response.status_code == 200:
                logger.info(f"Ntfy connection test successful. Using topic: {self.topic}")
                
                # Send a test notification
                test_success = self.send_message(
                    "Bot notification system is working properly ✅", 
                ...
                if test_success:
                    ...
                    logger.warning("Test notification failed, but connection is OK")
                    
                return True
            else:
                logger.error(f"Ntfy test failed with status code: {response.status_code}")
                self.enabled = False
                return False
                
        except Exception as e:
            logger.error(f"Error testing Ntfy connection: {e}")
            self.enabled = False
            return False
    ...
    def send_message(self, message, message_type='info', title=None):
        ...
        try:
            if not self.enabled or not self.topic or not self.base_url:
            ...
            logger.info(f"Sending Ntfy notification to {self.topic}")
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Failed to send Ntfy notification: {response.status_code} - {response.text}"
...
class PrecisionManager:
    ...
    def adjust_quantity(self, symbol, quantity):
        ...
        try:
            symbol_info = self.get_symbol_info(symbol)
            if not symbol_info:
                logger.warning(f"No precision info for {symbol}, using default")
...
class DynamicStopLoss:
    ...
    def calculate_support_resistance(self, df):
        ...
        try:
            ...
            if df_with_atr['atr'].isna().all() or df_with_atr['atr'].iloc[-1] == 0:
                ...
                logger.warning(f"Using default ATR: {default_atr:.4f}")
    ...
    def get_default_stop_loss(self, symbol, entry_price, direction):
        ...
        logger.warning(f"Using safe default stop loss for {symbol}")
    ...
    def cleanup_pending_trades(self):
        ...
        try:
            ...
            if pending_trades:
                ...
                if self.notifier and self.notifier.enabled and pending_trades:
                    message = f"Pending Trade Cleanup\nDetected and cleaned {len(pending_trades)} pending trades"
    ...
    def execute_trade(self, signal_data):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error setting leverage for {symbol}")
            ...
            if order and order.get('orderId'):
                ...
                try:
                    ...
                    logger.warning(f"Cannot get execution price for {symbol}")
                ...
                if self.notifier and self.notifier.enabled:
                    message = (
                        f"New Trade Executed - Level {trade_level}\n"
                        f"Symbol: {symbol}\n"
                        ...
                        f"Automatic trade management started\n"
        ...
        try:
            ...
            for position in positions:
                if position['symbol'] == symbol:
                    position_amt = float(position['positionAmt'])
                    if position_amt != 0:
                        logger.warning(f"Remaining unclosed position for {symbol}: {position_amt}")
    ...
    def send_heartbeat(self):
        ...
        try:
            ...
            if (current_time - self.last_heartbeat).seconds >= 7200:
                message = (
                    f"Heartbeat - Integrated Bot Working Successfully\n"
...
class UnifiedTradeBot:
    ...
    def send_performance_report(self):
        ...
        try:
            ...
            message = (
                f"Integrated Bot Performance Report\n"
...
def main():
    ...
    try:
        ...
        if bot.notifier and bot.notifier.enabled:
            message = (
                "Starting Integrated Bot v1.0\n"
                f"Role: Integrated System for Execution and Management\n"
                f"Features:\n"
            ...
            logger.warning("Ntfy notifier is disabled. Notifications will not be sent.")
